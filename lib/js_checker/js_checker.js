...
function isFreeLicensed(script){
    var magnets, re;
...
JsChecker.prototype.searchJs = function(jsCode, resultReady, url) {
    ...
    try {
        ...
        // if not external, parse using ChromeWorker.
        if (url === null){
            ...
            setHashCallback(
                ...
                console.debug("Analizing external script: "+url);
                this.parseTree = {};
                if (isFreeLicensed(jsCode))
                    this.parseTree.freeTrivialCheck = types.trivialWithComment(
                        "This script is under free software licenses: "+url);
                else
                     this.parseTree.freeTrivialCheck = types.nontrivialWithComment(
                         "This external script is not under free software licenses: "+url);
...
JsChecker.prototype.walkTreeComplete = function(result) {
    ...
    if (this.nontrivialness.type === checkTypes.NONTRIVIAL) {
        ...
        this.getCheckerResult(
                types.trivialWithComment("This script is trivial"));
    }
};
